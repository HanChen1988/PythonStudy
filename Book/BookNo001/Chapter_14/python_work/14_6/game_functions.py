...
def start_game(ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    if not stats.game_active:
        ...
        # 重置记分牌图像
        sb.prep_images(stats.score, 'score', 'number', '1')
        sb.prep_images(stats.high_score, 'high_score', 'number', '1')
        sb.prep_images(stats.level, 'level', 'number', '0')
        sb.prep_images(stats.ships_left, 'ship', 'image')
...
def store_high_score(ai_settings, stats):
    """存储最高分到文件中"""
    with open(ai_settings.text_file_path, 'w') as f_obj:
